...
基于 LILYGO [T-Display-S3](https://github.com/Xinyuan-LilyGO/T-Display-S3)（非触摸版）的桌面监控摆件固件：LVGL 界面实时显示 iKuai 路由器 WAN 状态、上下行速率与滚动趋势曲线。UI 采用酷态科（CUKTECH）小屏设计语言：纯黑底 + D-DIN Italic 大数字 + 彩色胶囊标签 + 无网格渐变曲线。
...
8. **无板载 WS2812 / TF 卡槽**：原项目的 RGB 状态灯与 TF 卡日志已移除——状态由屏幕左上角状态点 + 文案承担（绿=在线、红=离线）。SD 卡引脚仅在加装 TF Shield 时可用。
